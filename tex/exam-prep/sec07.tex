...
\begin{derivationbox}[Born 近似与分波法的系统对比]
...
\begin{derivationbox}[复变围道、主值积分与 Green 函数——系统梳理]
...
\begin{derivationbox}[解析延拓与积分路径的选择]
...
\begin{cheatbox}[Born 近似统一公式与速查表]
...
\begin{derivationbox}[傅里叶积分的详细计算]
...
\begin{derivationbox}[几何因子 $\vec{q}\cdot\vec{a}$ 的完整展开]
...
\begin{derivationbox}[与 1D 双 $\delta$ 势垒的数学联系]
...
\begin{derivationbox}[结构因子 $F(\vec{q})$ 与共振条件]
...
\begin{derivationbox}[``截面'' 家族：各种微分截面的定义]
...
\begin{derivationbox}[光学定理（Optical Theorem）]
...
\begin{derivationbox}[Born 近似 vs 光学定理：何时用哪个？]
