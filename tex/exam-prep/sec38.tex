...
\textbf{概念铺垫}：
\begin{itemize}[nosep]
  \item \textbf{应力} = 力/面积。压应力是"挤压"（正值），张应力是"拉伸"（负值，或说方向向外）；
  \item \textbf{抗张强度} = 材料能承受的最大张应力。超过这个值，晶体就会断裂；
  \item 在势能曲线中，$p = -\mathrm{d}U/\mathrm{d}V$。当 $r > r_0$ 时，$U$ 随 $r$ 增大而上升，$\mathrm{d}U/\mathrm{d}V > 0$，所以 $p < 0$（张应力）。
\end{itemize}
