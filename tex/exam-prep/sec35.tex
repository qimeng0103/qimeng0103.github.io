...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{images/nacl_2d_views.png}
\caption{NaCl 晶胞的三种画法。\textbf{左}：正视图（沿 $z$ 轴看）——只需画一个正方形，角上和面心画圈（Na$^+$），棱中点画叉（Cl$^-$）；\textbf{中}：俯视图（沿 $y$ 轴看）——同样的规则；\textbf{右}：轴测简图——用斜二测画法，先画前面的正方形，再向后上方拉三条平行线。考场上推荐先画左或中的正视图/俯视图，最容易上手。}
\label{fig:nacl-2d-views}
\end{figure}
