...
\end{equation}
对于一维链，``体积''即链长 $L=2Na$（$N$ 为原胞数，固定不变），故 $\gamma=-\partial\ln\omega/\partial\ln L$。长波声学支频率满足 $\omega=v_p q\propto\sqrt{\beta}$（$q$ 由固定边界条件确定，不随 $L$ 变），因此
\begin{equation}
\frac{\partial\ln\omega}{\partial\ln L}=\frac{1}{2}\frac{\partial\ln\beta}{\partial\ln L}
\quad\Longrightarrow\quad
\gamma=-\frac{1}{2}\frac{\partial\ln\beta}{\partial\ln L}=-\frac{L}{2\beta}\frac{d\beta}{dL}.
\end{equation}
又因为 $\beta=u''(a)$ 且 $a=L/(2N)$，有
\begin{equation}
\frac{d\beta}{dL}=u'''(a)\frac{da}{dL}=\frac{u'''(a)}{2N}=\frac{a\,u'''(a)}{L},
\end{equation}
代入即得一维链的微观公式（注意 $1/2$ 因子的来源正是 $\omega\propto\sqrt{\beta}$）：
\begin{equation}
