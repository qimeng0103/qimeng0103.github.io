...
\textit{标准定义}（一维链）：
\begin{equation}
\gamma=-\frac{a}{2}\frac{d\ln\omega}{da}=-\frac{a}{2\beta}\frac{d\beta}{da}.
\end{equation}

由于长波声速 $v_p\propto\sqrt{\beta a^2/M}$，频率 $\omega=v_pq\propto\sqrt{\beta}$（$q$ 由边界条件固定，与 $a$ 无关），故 $d\ln\omega/da=\frac{1}{2\beta}d\beta/da$。

更直接地，格林艾森常量可由势函数的高阶导数表示：
\begin{equation}
\gamma=-\frac{a}{2}\frac{u'''(a)}{u''(a)}.
\end{equation}
