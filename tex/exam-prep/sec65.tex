...
想象一块完整的晶体，中间某一层挖掉了一个圆盘区域（空位圆片）。这个圆盘的上、下两层原子失去了中间的``粘合''，就像一层楼板被抽掉：
\begin{itemize}
    \item 如果圆片很小：上下层原子可以通过弹性变形``搭桥''，自己愈合
    \item 如果圆片很大：上下层原子够不着对方，圆盘边缘必然塌陷
\end{itemize}
塌陷后，圆盘边缘的一圈原子面发生\textbf{错排}——不再是规则的晶格排列，这就是\textbf{位错环}（一条闭合的位错线）。

\textbf{位错为什么会有``符号''？}

刃位错的``符号''由\textbf{多余半原子平面}的方向决定：
\begin{itemize}
    \item \textbf{正刃位错}（符号 ``$\perp$''）：多余半平面\textbf{从上往下}插入，上方原子受\textbf{压}，下方受\textbf{张}
    \item \textbf{负刃位错}（符号 ``$\top$''）：多余半平面\textbf{从下往上}插入，上方原子受\textbf{张}，下方受\textbf{压}
\end{itemize}

\textbf{位错环上的符号分布}：

空位圆片塌陷时，圆片\textbf{内侧}的原子被``挤进来''（半平面插入）→ 形成一段\textbf{正刃位错}；圆片\textbf{外侧}的原子被``拉出去''（半平面缺失）→ 形成一段\textbf{负刃位错}。一圈位错环上，正、负符号交替出现（各约占一半）。

当横截面穿过位错环时，交于\textbf{两点}：一点是正刃位错，另一点是负刃位错。这就是 $\rho=2/S$ 中``$2$''的来源。
...
过饱和空位若均匀分布，能量较高；聚集成圆片后，虽然多了``表面''（圆片上下边界），但总的畸变能降低。圆片崩塌形成位错环是能量最低的路径。铸造金属或从熔体生长的晶体中，位错密度常达 $10^{6}\sim 10^{8}\,\mathrm{cm^{-2}}$，其主要来源正是此机制。
\end{insightbox}
...
\textbf{对比}：
\begin{itemize}
    \item \S\ref{sec:fcc-slip-dislocation}（范性形变）：位错密度由宏观滑移量估算，$\rho = \Delta L/(bS)$
    \item \S\ref{sec:vacancy-condensation}（空位崩塌）：位错密度由几何交截决定，$\rho = 2/S$
\end{itemize}
\end{thinkbox}
