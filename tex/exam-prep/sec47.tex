...
二维晶格有 $N$ 个原子，每个原子 2 个振动自由度，总模式数 $2N$。
经典能量均分：每个模式的平均势能 $= \frac12 k_BT$，故晶格总势能：
\begin{equation}
\langle U\rangle = 2N \times \frac12 k_BT = Nk_BT.
\label{eq:total-potential-2d}
\end{equation}

在简谐近似下，总势能也可写成单原子位移的二次型。对本题所给的色散关系，高温极限下的严格统计力学结果为：
\begin{equation}
\langle U\rangle = \frac12 N\beta\langle r^2\rangle.
...
\textit{检验}：一维单原子链（1 个自由度/原子）总势能 $=\frac12Nk_BT$，对应 $\langle x^2\rangle=k_BT/\beta$；二维增加一个独立方向，系数自然变为 2。
\end{derivationbox}
