...
\textbf{三维 $D(\omega)\propto\omega^2$ 的来源}

在 $\vec q$ 空间中，每个允许的 $\vec q$ 点占体积 $(2\pi)^3/V$。半径 $q$、厚度 $dq$ 的球壳内有 $Vq^2\,dq/(2\pi^2)$ 个 $\vec q$ 点。乘以 3 支声学模，再代入 $\omega=vq$：
\[
D(\omega)\,d\omega = \frac{3V}{2\pi^2}\Bigl(\frac{\omega}{v}\Bigr)^2\frac{d\omega}{v} \quad\Longrightarrow\quad D(\omega)=\frac{3V\omega^2}{2\pi^2v^3}.
\]
\textit{本质}：这是 3 维空间中``球壳面积 $\propto q^2$''的几何结果。
