...
\textbf{拓展延伸：}
\begin{itemize}
    \item \textbf{复数参数} $\xi = r e^{i\theta}$：此时 $S^\dagger(\xi) a S(\xi) = a \cosh r - a^\dagger e^{i\theta} \sinh r$。定义旋转后的坐标轴 $X_{\theta/2} = X \cos(\theta/2) + P \sin(\theta/2)$ 和 $P_{\theta/2} = -X \sin(\theta/2) + P \cos(\theta/2)$，则有 $S^\dagger(\xi) X_{\theta/2} S(\xi) = e^{-r} X_{\theta/2}$，$S^\dagger(\xi) P_{\theta/2} S(\xi) = e^{r} P_{\theta/2}$。标度变换的方向不再是坐标轴 $X, P$，而是相空间中旋转了 $\theta/2$ 角度的倾斜轴。
    \item \textbf{BdG 变换联系}：玻色子配对哈密顿量 $H = \omega a^\dagger a + \frac{\Delta}{2}(a^2 + (a^\dagger)^2)$ 的对角化，本质上就是这个压缩变换的玻色子版本。其关键区别在于：为了保持玻色对易关系 $[b, b^\dagger] = 1$，变换系数必须满足 $u^2 - v^2 = 1$，因此用双曲函数 $(\cosh r, \sinh r)$ 参数化；而费米子 BdG 变换为了保持反对易关系 $\{c, c^\dagger\} = 1$，系数满足 $u^2 + v^2 = 1$，因此用三角函数 $(\cos\theta, \sin\theta)$ 参数化。
\end{itemize}
\end{insightbox}
