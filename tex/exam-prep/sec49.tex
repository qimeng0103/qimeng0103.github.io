...
由守恒方程反解：
\begin{align}
    \hbar\omega(\vec q) &= \bigl|\,E - E'\,\bigr| = \frac{\hbar^2}{2M}\bigl|\,k^2 - k'^2\,\bigr|, \\
    \vec q &= \pm(\vec k - \vec k' + \vec G) \equiv \pm(\vec Q + \vec G).
\end{align}

通过测量不同 $\vec Q$ 方向上的能量转移 $\Delta E = E - E'$，当散射强度出现峰值（共振）时，即对应某支声子模式的能量和波矢。
\end{derivationbox}
