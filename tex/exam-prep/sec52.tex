...
这是一个常见的概念困惑。\textbf{``各向同性''不等于``只有一种波''}。各向同性只意味着弹性性质不随方向变化（波速与传播方向无关），但纵波和横波由于\textbf{偏振方式不同}、激发的弹性形变类型不同，仍然有不同的波速。

在连续介质力学中，各向同性弹性固体的位移场 $\vec u(\vec r,t)$ 满足标准的弹性波动方程。通过 Helmholtz 分解 $\vec u=\vec u_l+\vec u_t$，其中
\begin{itemize}
    \item $\nabla\times\vec u_l=0$（无旋）：\textbf{纵波}，质点振动方向与传播方向平行，引起压缩/膨胀（改变体积）
    \item $\nabla\cdot\vec u_t=0$（无散）：\textbf{横波}，质点振动方向与传播方向垂直，引起纯剪切（不改变体积）
\end{itemize}
...
\end{equation}
其中 $B$ 为体积模量，$\mu$ 为剪切模量，$\rho$ 为密度。由于 $B>0$，总有 $v_l>v_t$（除非 $\mu=0$ 的液体，此时不存在横波）。

横波有两个正交的偏振方向（均垂直于 $\vec q$），各向同性下二者简并，故共有\textbf{1支纵波 + 2支横波}。

\textit{与真实晶体的对比}：在各向异性晶体（如立方晶系）中，纵波和横波的波速还会随传播方向变化，横波的两个偏振也可能不简并。德拜模型的``各向同性''假设是把方向依赖性平均掉了，但保留了偏振（纵/横）这个根本区别——因为纵波压缩和横波剪切是两种物理上完全不同的形变模式。
