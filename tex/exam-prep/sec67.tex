...
\begin{insightbox}[平均场近似的物理意义]
\textbf{平均场近似}（Mean Field Approximation）的核心思想是：\textbf{忽略局域涨落，用整体的统计平均代替每个格点周围的实际环境}。

具体来说：
\begin{itemize}
    \item 系统中共有 $N$ 个格点，其中 $N_A$ 个被 $A$ 原子占据，$N_B$ 个被 $B$ 原子占据
    \item 如果不考虑原子间的关联（即原子随机分布），那么任意挑一个格点，它是 $A$ 原子的概率就等于 $A$ 原子在整体中的占比 $N_A/N$
    \item 同理，它是 $B$ 原子的概率为 $N_B/N$
\end{itemize}

\textbf{类比理解}：想象一个装有 $N_A$ 颗红豆和 $N_B$ 颗绿豆的大袋子。你伸手随机抓一颗，抓到红豆的概率就是 $N_A/(N_A+N_B) = N_A/N$。平均场近似就相当于说：\textbf{每个原子周围的近邻环境，就像从袋子里随机抓出来的一样}，不受该原子本身是 $A$ 还是 $B$ 的影响。

\textbf{什么时候不成立？} 低温下原子倾向于抱团（同类原子聚集），此时一个 $A$ 原子周围的 $A$ 原子比例会\textbf{显著高于}整体浓度 $N_A/N$。这时平均场近似失效，需要用更精确的方法（如伊辛模型的严格解或蒙特卡洛模拟）。但在本题中，平均场近似足够给出定性正确的物理图像和临界温度。
\end{insightbox}
...
F(x) &= -3NJx^2 - Nk_{\!B}T\ln 2 \notag\\
     &\quad + \frac{Nk_{\!B}T}{2}\Bigl[(1+x)\ln(1+x) + (1-x)\ln(1-x)\Bigr].
\end{align}
