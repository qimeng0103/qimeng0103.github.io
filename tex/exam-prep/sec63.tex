...
\textit{为什么是 ``切'' 应力？} 滑移是晶面间的\textbf{相对平移}，上层原子沿 $[100]$ 方向（平行于滑移面）移动。恢复力 $F$ 也沿这个切向。与压强（法向应力，力垂直于面）不同，\textbf{切应力}定义为切向力除以面积：$\tau = F_{\parallel}/S$。想象推一叠扑克牌：推力平行于牌面，使各层错开——这就是切应力驱动的滑移。
