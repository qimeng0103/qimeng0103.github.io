...
过程 & 光子能量变化 & 频率变化 & 颜色说法 \\
\midrule
\textbf{斯托克斯}（发射声子） & 光子\textbf{损失}能量 & $\omega\downarrow$ & \textbf{红移} \\
\textbf{反斯托克斯}（吸收声子） & 光子\textbf{获得}能量 & $\omega\uparrow$ & \textbf{蓝移} \\
\bottomrule
\end{tabular}
\end{center}
...
\textit{记忆技巧}：
\begin{enumerate}
    \item \textbf{能量口诀}：斯托克斯 = ``斯托''谐音``损失''→光子损失能量→频率降低→\textbf{红移}；反斯托克斯 = ``反''→反过来→光子获得能量→频率升高→\textbf{蓝移}。
    \item \textbf{颜色口诀}：可见光谱中，红光波长\textbf{最长}、频率最低；蓝光/紫光波长\textbf{较短}、频率较高。``红长蓝短''——红移就是往长波方向移，蓝移就是往短波方向移。
    \item \textbf{波长变化}：看公式中的 $\pm$ 号。$\Delta\lambda=\lambda-\lambda_0=\pm\cdots$。斯托克斯取 $+$（因为 $1/\lambda-1/\lambda_0=-\cdots$，通分后 $\lambda>\lambda_0$），波长变\textbf{长}→红移；反斯托克斯取 $-$，波长变\textbf{短}→蓝移。
\end{enumerate}

\textbf{4. 为什么 $|\vec k|\approx|\vec k_0|$ 是良好近似？}

\textbf{3. 为什么 $|\vec k|\approx|\vec k_0|$ 是良好近似？}

以典型值估算：取 $\lambda_0 = 500\,\mathrm{nm}$，$n=1.5$，$v = 5\times10^{3}\,\mathrm{m/s}$，$\theta = 90^\circ$：
\begin{align}
\Delta\lambda &\approx \frac{2\times1.5\times5\times10^{3}}{3\times10^{8}}\times500\times10^{-9}\times\sin45^\circ \\
&\approx 1.8\times10^{-11}\,\mathrm{m} = 0.018\,\mathrm{nm}.
\end{align}

相对频移 $\Delta\lambda/\lambda_0 \sim 10^{-5}$，确实极小。这意味着光子几乎把所有能量带走，只``施舍''给声子一点点能量——这正是弹性近似成立的基础。
