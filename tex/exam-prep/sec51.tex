...
\begin{itemize}
    \item \textbf{斯托克斯过程}（发射声子）：需要晶体中已存在可被激发的振动模式，强度正比于 $n_\mathrm{BE}+1$
    \item \textbf{反斯托克斯过程}（吸收声子）：需要预先存在热声子，强度正比于 $n_\mathrm{BE}$
\end{itemize}

因此，在室温下斯托克斯线通常比反斯托克斯线\textbf{更强}。低温下 $n_\mathrm{BE}\to0$，反斯托克斯线几乎消失。这一强度比是测量样品温度的常用手段。
