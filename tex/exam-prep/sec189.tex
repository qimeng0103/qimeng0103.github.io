...
在该子空间内，哈密顿量可按对称性分块：$\bm{k}_1$ 与 $\bm{k}_4$ 的差矢量为 $\bm{G}_{11}=\frac{2\pi}{a}(1,1)$，$\bm{k}_2$ 与 $\bm{k}_3$ 的差矢量为 $\bm{G}_{-1,1}=\frac{2\pi}{a}(-1,1)$，二者均出现在势场的傅里叶展开中。每个 $2\times2$ 块的非对角元为 $-U$。
