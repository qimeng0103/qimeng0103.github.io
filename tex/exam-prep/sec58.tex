...
\textbf{对比}：
\begin{itemize}
    \item \textbf{单质肖特基缺陷}（单一原子晶体）：仅一类空位，$W=\binom{N}{n}$，故 $n = N e^{-u/k_{\!B}T}$，指数上无 $1/2$。
    \item \textbf{离子晶体肖特基缺陷}（正负离子空位成对）：$W=\binom{N}{n}^2$，与弗仑克尔缺陷形式相同，$n = N e^{-u/2k_{\!B}T}$，指数上也有 $1/2$。
\end{itemize}
\end{insightbox}
