...
\begin{enumerate}
    ...
    \end{itemize}
\end{enumerate}
\textit{间隙位置的几何类型与数目}（以 $N$ 个原子为基准）：

晶体中的间隙位置按\textbf{配位数}（周围最近邻原子数）分为两类：
\begin{itemize}
    \item \textbf{八面体间隙}：周围有 $6$ 个最近邻原子，呈八面体配位。位于晶胞的体心、面心或棱心位置。
    \item \textbf{四面体间隙}：周围有 $4$ 个最近邻原子，呈四面体配位。位于晶胞内部的原子 tetrahedron 空隙中。
\end{itemize}
...
简单立方（sc） & $N$（体心） & $2N$ \\
面心立方（fcc） & $N$（棱心+体心） & $2N$ \\
体心立方（bcc） & $3N$（面心+棱心） & $6N$ \\
六角密堆（hcp） & $N$ & $2N$ \\
\bottomrule
\end{tabular}
\end{center}
\vspace{2pt}
\textbf{为什么一个结构有两种间隙？} 间隙位置的分类源于\textbf{对称性}：晶体中不同位置的几何环境不同（配位数、到周围原子的距离不同），形成能也不同。小原子（如 C、N、H）通常占据八面体间隙，因为空间较大；极小原子可能进入四面体间隙。

弗仑克尔缺陷中 $N'$ 指\textbf{可被占据的间隙位置总数}。考试中若未指明间隙类型，默认按题设条件或取 $N'=N$。
