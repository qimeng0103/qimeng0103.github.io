...
\textit{基本原理。}本题通过比较两种独立测量手段的结果来推断缺陷浓度：
\begin{itemize}
    \item \textbf{X射线衍射法}：测量晶格常数 $a$，计算晶胞体积 $v_{\mathrm{cell}}=a^3$。这个体积是含空位膨胀后的\textbf{实际体积} $V$（大于理想体积 $V_0$）。若\textbf{假设晶体完整}（晶胞内原子数仍为 $n$），则由测得的宏观密度 $\rho$ 反推的``分子量''为
    \begin{equation}
    m' = \frac{\rho\,v_{\mathrm{cell}}\,N_{\!A}}{n},
    \end{equation}
    称为\textbf{表观分子量}。实际晶胞内只有 $n(1-c)$ 个原子（$c$ 为空位浓度），但反推时假设有 $n$ 个，故 $m'$ 被\textbf{低估}为 $m(1-c)$。
    \item \textbf{化学法}：通过化学分析确定晶体中各元素的原子比例，直接计算出\textbf{真实分子量} $m$（与体积无关）。
\end{itemize}

\textit{定量关系。}设空位浓度为 $c=n/N$。由 Step 2，体积相对膨胀 $\Delta V/V_0 = c$，即 $V = V_0(1+c)$。

实际晶体的质量等于完整晶体质量乘以 $(1-c)$（少了 $c$ 比例的原子），故实际密度
\begin{equation}
\rho = \frac{M}{V} = \frac{M_0(1-c)}{V_0(1+c)} \approx \rho_0(1-2c)\quad(c\ll 1).
\end{equation}

表观分子量 $m'$ 用实际密度 $\rho$ 和膨胀体积 $V$ 计算，相当于假设原子完整：
\begin{equation}
m' = \frac{\rho V}{n/N_{\!A}} = \frac{M_0(1-c)}{V_0(1+c)} \cdot V_0(1+c) \cdot \frac{N_{\!A}}{n} = \frac{M_0(1-c)}{n/N_{\!A}} = m(1-c).
\end{equation}
\textit{注意}：密度 $\rho$ 中的体积膨胀因子 $(1+c)$ 与 $m'$ 公式中的 $V$ 恰好抵消，故 $m'$ 偏小的唯一原因是实际原子数减少了 $(1-c)$。真实分子量 $m$ 对应完整晶体。因此
\begin{equation}
\frac{m}{m'} = \frac{1}{1-c} \approx 1+c = \frac{V}{V_0}.
\end{equation}
\textbf{关键结论}：真实分子量与表观分子量之比等于实际体积与理想体积之比，直接给出缺陷浓度 $c = m/m' - 1$。
...
\end{equation}
\textit{注}：NaCl 肖特基缺陷形成能的实验值约为 $2.0\sim 2.5\,\mathrm{eV}$。本题数据为简化处理，得到的 $1.44\,\mathrm{eV}$ 仅为估算数量级。
\end{derivationbox}
