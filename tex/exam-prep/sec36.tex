...
\begin{insightbox}{元胞手绘——从"看不清"到"一眼认出"}

\textbf{核心痛点}：只画一个晶胞时，角上的原子只占 1/8、棱上的只占 1/4，看起来支离破碎，没有"包裹在内部"的整体感。

\textbf{解决方法是 2$\times$2 框选法}：
\begin{enumerate}[nosep]
  \item \textbf{画格点阵列}：不要只画一个正方形/立方体，先画 $2\times 2$ 的四个正方形（2D）或 $2\times 2\times 2$ 的八个立方体（3D）；
  \item \textbf{填完整原子}：所有原子都用完整符号（圈/叉）画出，\textbf{不要画"缺角"的原子}——角上的原子画完整圆圈，不要被边界切割；
  \item \textbf{粗线框选一个晶胞}：用粗实线框出阵列中\textbf{任意一个}元胞的范围。这个框就是你的"答案区域"；
  \item \textbf{在框内标注分摊}：框内每个原子旁边标权重（角上 $\times\frac{1}{8}$、棱上 $\times\frac{1}{4}$、面上 $\times\frac{1}{2}$、体内 $\times 1$）。框外的原子是\textbf{背景参照系}，不用标权重。
\end{enumerate}

\textbf{为什么有效}：在 $2\times 2$ 阵列中，内部的原子被完整包围，边界上的原子虽然被框切到，但因为相邻晶胞的存在，你能直观地看到"这个原子被几个晶胞共享"。框出一个晶胞后，数框内的加权原子数就是答案。

\textbf{具体操作示例——简单立方}：
\begin{itemize}[nosep]
  \item 画 $2\times 2\times 2 = 8$ 个立方体拼成一个大立方体；
  \item 在每个格点画一个完整的小圆圈（8 个角 + 12 个棱中 + 6 个面心 + 1 个体心 = 27 个格点）；
  \item 用粗线框出大立方体\textbf{左下角靠前}的那一个小立方体；
  \item 框内有：8 个角（各 $\frac{1}{8}$）、12 条棱（各 $\frac{1}{4}$）、6 个面（各 $\frac{1}{2}$）、1 个体心（$\times 1$）；
  \item 数：$8\times\frac{1}{8} + 12\times\frac{1}{4} + 6\times\frac{1}{2} + 1\times 1 = 1+3+3+1 = 8$ 个原子。
\end{itemize}

\textbf{关键认知}：\textbf{画多了不是错，画少了看不清才是错}。考试阅卷老师看的是框内的分摊计算是否正确，不会因为你多画了背景而扣分。
\end{insightbox}
