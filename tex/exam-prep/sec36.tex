...
\vspace{0.5em}
\textbf{3D 简法——完整原子 + 直接标注}

$3\times 3\times 3 = 27$ 个立方体考场上画不动，换最简策略：
\begin{enumerate}[nosep]
  \item \textbf{画一个立方体}（斜二测或正视图都可以），只画可见的边（通常 3--6 条实线即可）；
  \item \textbf{画完整原子}：在 8 个角、12 条棱中、6 个面心、1 个体心的位置画完整符号（圈/叉），\textbf{不要被立方体边界切割}；
  \item \textbf{直接标权重}：每个原子旁边写分摊系数（角 $\times\frac{1}{8}$、棱 $\times\frac{1}{4}$、面 $\times\frac{1}{2}$、体 $\times 1$）。
\end{enumerate}

\textbf{不需要画虚线、不需要画相邻晶胞}——阅卷老师只看框内的分摊计算是否正确。

\textbf{示例——简单立方 3D}：
\begin{itemize}[nosep]
  \item 画一个立方体（前面一个正方形 + 顶边和右边用斜线拉出深度感）；
  \item 8 个角画圆圈，旁标 $\times\frac{1}{8}$；
  \item 12 条棱中点画符号，旁标 $\times\frac{1}{4}$；
  \item 6 个面心画符号，旁标 $\times\frac{1}{2}$；
  \item 体心画符号，旁标 $\times 1$；
  \item 数：$8\times\frac{1}{8} + 12\times\frac{1}{4} + 6\times\frac{1}{2} + 1 = 1+3+3+1 = \boxed{8}$。
\end{itemize}

\textbf{关键认知}：3D 中不要追求"画出一个完全被包围的晶胞"（那需要 $3\times 3\times 3$ 阵列，不现实）。\textbf{最简方案}：画一个立方体 + 完整原子 + 直接标注权重。阅卷老师看的是计算，不是美术。
\end{insightbox}
