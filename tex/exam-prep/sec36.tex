...
\begin{insightbox}{考场手绘晶体结构的技巧}
考场上不需要画完美的 3D 球体，\textbf{用符号代替颜色}即可：

\textbf{NaCl 结构}（重点掌握）：
\begin{enumerate}[nosep]
  \item 先画一个立方体框架（只画可见的三条棱，其余省略）；
  \item 角上画小圆圈 $\circ$（Na$^+$），面心画小圆圈 $\circ$（Na$^+$）；
  \item 棱中点画叉 $\times$（Cl$^-$），体心画叉 $\times$（Cl$^-$）。
\end{enumerate}
