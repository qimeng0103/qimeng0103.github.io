...
\textbf{一、密铺（Close Packing）——分层错位法}

密堆结构（fcc、hcp）的核心是\textbf{原子层的六方排列}。手绘时不要试图一次性画 3D，而是严格按层堆叠：
\begin{enumerate}[nosep]
  \item \textbf{第一层（A 层）}：画一排紧密相切的圆，第二排嵌入第一排的凹陷处，形成六方蜂窝状。每 7 个原子画出 1 个中心原子 + 6 个近邻即可表达规律；
  \item \textbf{第二层（B 层）}：原子中心正对 A 层的三原子凹陷处（即 A 层原子围成的正三角形中心上方）；
  \item \textbf{第三层}：若回到 A 层原子上方 → \textbf{hcp}（ABAB\dots）；若错开到 C 层位置 → \textbf{fcc}（ABCABC\dots）。
\end{enumerate}

记忆口诀：\textbf{"六方打底，凹陷堆高；ABAB 是六角，ABC 是立方面心"}。fcc 的密堆面恰好是 \{111\} 面，这也是fcc 晶体解理面的来源。

\vspace{0.5em}
\textbf{二、Wigner-Seitz 原胞——中垂面切割法}

WS 原胞是\textbf{倒空间中第一布里渊区的实空间对应物}，手绘时遵循"连线 $\to$ 取中点 $\to$ 作垂面"三步：
\begin{enumerate}[nosep]
  \item 标出中心格点和所有最近邻格点（sc 有 6 个，bcc 有 8 个，fcc 有 12 个）；
  \item 用直线连接中心到每个最近邻，在所有连线的中点处作\textbf{垂直平分面}（2D 手绘时就是垂直平分线）；
  \item 这些平面围成的最小凸多面体就是 WS 原胞。
\end{enumerate}
...
\begin{tabular}{|l|c|l|}
\hline
\textbf{格子} & \textbf{最近邻数} & \textbf{WS 原胞形状} \\
\hline
简单立方 sc & 6 & 立方体（与原胞相同） \\
体心立方 bcc & 8 & \textbf{截角八面体}（14 个面：6 个正方形 + 8 个正六边形） \\
面心立方 fcc & 12 & \textbf{菱形十二面体}（12 个菱形面） \\
\hline
\end{tabular}
\end{center}

\textbf{手绘技巧}：画 bcc 的截角八面体时，先画一个正方体，再把 8 个角切掉（切面是正六边形的 8 个顶点方向）。fcc 的菱形十二面体可以想象成两个四角锥扣在八面体两端，但更简单的是直接记住它有 12 个全等的菱形面。
...
\textbf{手绘策略}：
\begin{enumerate}[nosep]
  \item 先画其中一套\textbf{完整的框架}（比如用圆圈表示 Cl$^-$ 的 fcc 格点）；
  \item 在平移后的位置用\textbf{另一种符号}（叉或实心点）标记第二种原子；
  \item 只画\textbf{一个晶胞}的范围，超出边界的原子用虚线或不画。
\end{enumerate}
