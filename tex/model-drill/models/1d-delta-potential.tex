...
\trainingpoint{6} \textbf{高斯试探波函数 vs 指数试探波函数}：对 $V(x)=-\gamma\delta(x)$，分别用高斯型 $\psi_\alpha\propto e^{-\alpha x^2}$ 和指数型 $\psi_\lambda\propto e^{-\lambda|x|}$ 做变分法。高斯试探给出 $E_{\text{var}}=-\frac{\mu\gamma^2}{\pi\hbar^2}$，指数试探给出精确解 $E_0=-\frac{\mu\gamma^2}{2\hbar^2}$。比较两者的优劣：指数型恰为精确解（$\pi>2$ 故变分结果更高），高斯型因在原点过于平滑而高估动能。
\end{drillbox}
