...
**客观知识呈现：**
- ❌ **禁止提及来源**：不出现作者姓名（如 "according to Sakurai", "Zeng Jinyan's book"）、书名、或章节号
- ✅ **纯粹知识内容**：直接呈现物理概念、数学推导、定理证明
- ✅ **英文全文**：所有正文内容必须使用英文（包括推导步骤和解释）

**推导完整性要求：**
- ❌ **禁止简化**：不允许 "It can be shown that...", "After some algebra...", "The result is..." 这类省略推导的表述
- ✅ **完整推导**：每一步数学变换都必须写出，包括中间步骤
- ✅ **详细解释**：每个方程的物理意义、每个近似条件的说明
- ✅ **边界条件**：所有匹配条件、连续性条件必须显式写出并求解
